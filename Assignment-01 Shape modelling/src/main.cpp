...
int main(int, char**)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawArrays(GL_TRIANGLES, 0, 1748*3);
        glUniform3f(vColor_uniform, 0.0, 0.0, 0.0);
        glDrawArrays(GL_LINE_STRIP, 0, 1748*3);
...
void createCubeObject(unsigned int &program, unsigned int &cube_VAO)
{
    ...
    // GLfloat cube_vertices[] = {2, 2, -2, -2, 2, -2, -2, -2, -2, 2, -2, -2, //Front
    //                2, 2, 2, -2, 2, 2, -2, -2, 2, 2, -2, 2}; //Back
...
void createParametricObject(unsigned int &program, unsigned int &shape_VAO)
{
    ...
    // Using a vector because it allows resizing of array during runtime.


    vector<float> points_vertices;
    ...
    for(int phi_step = 0;phi_step<total_phi_steps;phi_step++)
    {
        ...
        if(1==phi_step)
        {
            for(int theta_step = 0;theta_step<total_theta_steps;theta_step++)
            {
                ...
                for(int i=0;i<3;i++) points_vertices.push_back(coordinates[i]);
    ...
    size_t nVertices = (points_vertices.size())/3; // No. of vertices of the shape
    cout<<"vertics = "<<nVertices<<"\n";
    cout<<"set GlDrawArray Parameter to -> "<<nVertices/3<<" * 3\n";
    GLfloat *shape_vertices = new GLfloat[points_vertices.size()];
    int del = 0;
    for(int i=0;i<points_vertices.size();i++)
    {
        shape_vertices[i] = points_vertices[i];
        // cout<<points_vertices[i]<<" ";
        // del++;
        // del%=3;
        // if(0==del) cout<<"\n";
    } 
